...
'''
This script will generate the required input file for dadi based on Stacks output data.  Required input is an output directory to write results, popmap file, plink ped file, plink map file, fasta file of consensus sequences for each locus (as generated by consensus_fasta.py), and comma separated list of ingroup populations (e.g. west,east,mexico,africa). Note that the position in PLINK for this version of Stacks is 1 position off, so this script corrects for the actual position (every position is actually +1). This format also does not take any information about the outgroup, and so is only appropriate for folded SFS calculations.
...
def main(argv):
	try:
		opts,args = getopt.getopt(argv,'ho:m:P:M:f:I:',)
	except getopt.GetOptError:
		print "dadi_format_converter.py -o <path to output file (default ./dadi_format.txt)> -m <Popmap file> -P <Plink ped file> -M <Plink map file> -f <Fasta file of consensus sequences for each locus> -I <Ingroup comma separated list>"
		sys.exit(2)
	...
	for loc in range(0,len(locusid)):
		...
		if sum([int(i) for i in allele2_dict[locusid[loc]]])>=sum([int(i) for i in allele2_dict[locusid[loc]]]):	
			line = [in_ref[locusid[loc]],out_ref[locusid[loc]],allele_vars[loc][1]]+allele2_dict[locusid[loc]]+[allele_vars[loc][0]]+allele1_dict[locusid[loc]]+[catid[loc],str(tagpos[loc])]
		else:
